...
import pygame
from chapter14_scoring.bullet import Bullet
from chapter14_scoring.alien import Alien
...
def check_events(ai_settings, screen, stats, sb,
    ...
    for event in pygame.event.get():  # 事件循环，所有鼠标和键盘事件都将促使for循环运行
        ...
        elif event.type == pygame.MOUSEBUTTONDOWN:  # 单击屏幕任何地方都是一个MOUSEBUTTONDOWN事件
            mouse_x, mouse_y = pygame.mouse.get_pos()  # 返回一个包含单击的x和y坐标的元组
...
def check_play_button(ai_settings, screen, stats, sb, play_button,
    ...
    # 单击play且game_active为False才开始新游戏，否则开始后单击play所在区域也会重新开始游戏
...
def check_bullet_alien_collisions(ai_settings, screen,
    ...
    if collisions:  # 记分
        # 每个值都是一个列表，表示被同一颗子弹击中的外星人，保证正确地记到击中每个外星人得分
...
def check_high_score(stats, sb):
    """检查是否诞生了最高分"""
    if stats.score > stats.high_score:  # 比较当前得分和最高分
